...
list.files(here::here("GA_dashboard/part_9/R")) %>%
    here::here("GA_dashboard/part_9/R", .) %>%
    purrr::walk(~source(.))
...
ui <- fluidPage(
    ...
    shinyjs::useShinyjs(),
    shiny::includeCSS(here::here("GA_dashboard/part_9/www/styles.css")),
    ...
    shiny::includeScript(here::here("GA_dashboard/part_9/www/scripts.js"))
...
server <- function(input, output) {
  ...
  output$gg <- shiny::renderUI({
    html <- div(
      class = "row eq-height",
      shiny::tagList(
        time_series_pageviews(web_data)[['cards_views_sessions']] %>%
          create_cards()
      )
    )
  })

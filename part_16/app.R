...
df <- dplyr::tibble(
  main_page = dplyr::tibble(
    viz = list(c("Page Views", "Device Category", "Session Duration", "Channel Groupings")),
    ids = list(c("a", "b", "c", "d"))
  ) %>% list(),
  sidebar = dplyr::tibble(
    viz = list(c("Bounce Rate", "Visitor Map", "Most Popular Posts")),
    ids = list(c("e", "f", "g"))
  ) %>% list()
...
connections$con_visualizations$drop()
connections$con_visualizations$insert(df)

# connections$con_visualizations$update(
#   query = '{"user": "pascal"}',
#   update = '{"$unset":{"main_page.0.df.1": 1}}'
# )
#
# connections$con_visualizations$update(
#   query = '{"user": "pascal"}',
#   update = '{"$pull":{"main_page.0.df": null}}'
# )
...
web_data <- readr::read_csv(here::here("GA_dashboard/part_16/data/web_data.csv"))
web_data_c <- readr::read_csv(here::here("GA_dashboard/part_16/data/search_console.csv"))

geo <<- readr::read_rds(here::here("GA_dashboard/part_16/data/map.rds"))
...
ui <- fluidPage(
    ...
    shinyjs::useShinyjs(),
    shinyjs::extendShinyjs(script = here::here("GA_dashboard/part_16/www/scripts.js"), functions = c("remove_plots_date_change")),
    shiny::includeCSS(here::here("GA_dashboard/part_16/www/styles.css")),
    ...
    shiny::includeScript(here::here("GA_dashboard/part_16/www/scripts.js"))
...
server <- function(input, output) {
  ...
  main_viz_server(
    ...
    main_page_info  = viz,
    delete_db_id    = shiny::reactive(input$delete_db),
